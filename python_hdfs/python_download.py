...
def import_fse(quandl_companies_list, quandl_start_date, quandl_end_date, hdfs_export_path, hdfs_connection):
    for company in quandl_companies_list:
        hdfs_export_filename = hdfs_export_path + "/" + company + "_" + quandl_end_date.replace("-", "_") + ".json"
        #hdfs_export_filename = "/root/Desktop/Big Data Data/python_hdfs/results/fse" + "/" + company + "_" + quandl_end_date.replace("-", "_") + ".json"
        #check if newest file already exists
        #if os.path.exists(hdfs_export_filename):
        if hdfs_connection.exists_file_dir(hdfs_export_filename):
            print("newest file for company " + company + " already exists")
            continue
        ...
        for val in vals:
            ...
            jsonobj = {}
            for value in val:
                jsonobj[headers[i]] = value
                i+=1
            jsons[str(j)] = jsonobj
            j+=1
        jsonstring = json.dumps(jsons)
        print("creating FSE File: " + hdfs_export_filename)
        hdfs_connection.create_file(hdfs_export_filename, jsonstring, permission=777)
        #file = open(hdfs_export_filename, "w+")
...
def import_infections(import_url, hdfs_connection, hdfs_path_infection):
    ...
    for value in jsonresponse:
        if value["continentExp"] == 'Europe':
            if geoID == value["geoId"] or geoID == "":
                jsonresult[value["dateRep"]] = value
                
            else:
                #filename = hdfs_path_infection + "/" + geoID + "_" + daterep.replace("/", "_") + ".json"
                #if not os.path.exists(filename):
                if not hdfs_connection.exists_file_dir(filename):
                    #file = open(filename, "w+")
                    #file.write(json.dumps(jsonresult))
                    #file.close()
                    hdfs_connection.create_file(filename, jsonresult)
                    print("created file " + filename)
                    jsonresult = {}
                    
                else:
                    print("there is already a up to date file existing for geoid: " + value["geoId"])
            geoID = value["geoId"]
            #filename = hdfs_path_infection + "/" + geoID + "_" + daterep.replace("/", "_") + ".json"
            filename = hdfs_path_infection + "/" + geoID + "_" + datetime.date.today().strftime("%Y_%m_%d") + ".json"
        else:
            if not jsonresult == {}:
                #if not os.path.exists(filename):
                if not hdfs_connection.exists_file_dir(filename):
                ...
                jsonresult = {}
            #print('No european country. Data will not be saved')
