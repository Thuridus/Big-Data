...
$$(".convert-form-to-data").on("click", function () {
  ...
  if (content.dateRange.indexOf("-") < 0) {
    ...
    var sql = `SELECT YEAR(dax.date) as year, ${grouptyp}(dax.date) as field, DATE_FORMAT(dax.date, '%d.%m.%Y') as day, sum(infects.${coronaTyp}) as corona, sum(dax.diff) as dax FROM infects INNER JOIN dax ON infects.date = dax.date WHERE dax.date BETWEEN CAST('${content.startDate}' AS DATE) AND CAST('${content.endDate}' AS DATE) AND country IN (${countrys}) GROUP BY YEAR(dax.date), ${grouptyp}(dax.date) ORDER BY YEAR(dax.date), ${grouptyp}(dax.date);`;
